...
#curl -c /tmp/cookie -X POST -k "$SERVER_URL/store/site/blocks/user/login/ajax/login.jag" -d "action=login&userName=$clouduser&password=$clouduserpass"

#appStatusCode=$(curl -w "%{http_code}" -b /tmp/cookie -X POST "$SERVER_URL/store/site/blocks/application/application.jag" -d "action=getApplication&applicationName=$APPLICATION_NAME")
#if [[ $appStatusCode = *"200"* ]]; then
#echo "application already exists"
#appHashId=$(curl -b /tmp/cookie -X POST "$SERVER_URL/store/site/blocks/application/application.jag" -d "action=getApplication&applicationName=$APPLICATION_NAME" |jq -r .hashId)
#echo $appHashId
#if [[ ! -z $appHashId ]] 
#then
